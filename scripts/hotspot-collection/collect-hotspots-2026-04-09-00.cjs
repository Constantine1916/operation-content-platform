#!/usr/bin/env node
// Hotspot collection script - 2026-04-09 00:00 UTC (08:00 Beijing)
...
async function main() {
  console.log('Starting hotspot collection for', DATE, TIME, '(08:00 Beijing)...');
  ...
  const outFile = path.join(outDir, `${DATE}_00.json`);
  ...
  if (fs.existsSync(radarDir)) {
    fs.writeFileSync(path.join(radarDir, `${DATE}_00.json`), JSON.stringify(data, null, 2), 'utf-8');
    console.log(`✅ Synced to radar: ${path.join(radarDir, `${DATE}_00.json`)}`);
  }
}
