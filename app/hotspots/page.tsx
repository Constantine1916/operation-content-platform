  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-400 tracking-[0.15em] uppercase">Hotspot News</p>
      </div>

      <div className="mb-6 flex gap-2 flex-wrap">
        <button onClick={() => setSelectedCategory('')} className={`px-3 py-1.5 rounded-full text-base font-medium transition-all ${selectedCategory === '' ? 'bg-gray-900 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:border-gray-400'}`}>全部</button>
        {categories.slice(0, 12).map(cat => (
          <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-3 py-1.5 rounded-full text-base font-medium transition-all ${selectedCategory === cat ? 'bg-gray-900 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:border-gray-400'}`}>{cat}</button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <>
            ...
            {hotspots.map((hotspot) => (
              <a key={hotspot.id} href={hotspot.url} target="_blank" rel="noopener noreferrer"
                ...
                <div className="flex items-center gap-2 mb-2">
                  {hotspot.热度 && <span className="px-2 py-0.5 rounded-full text-base font-medium bg-gray-50 text-gray-600 border border-gray-100">🔥 {hotspot.热度}</span>}
                  {hotspot.category && <span className="px-2 py-0.5 rounded-full text-sm bg-gray-50 text-gray-500 border border-gray-100">{hotspot.category}</span>}
                  <span className="text-sm text-gray-400">{formatDate(hotspot.collected_date)}</span>
                </div>
                <h3 className="text-base font-medium text-gray-800 group-hover:text-gray-900 line-clamp-2 mb-2">{hotspot.title}</h3>
                {hotspot.summary && <p className="text-sm text-gray-500 line-clamp-2 mb-2">{hotspot.summary}</p>}
                {hotspot.source && <div className="text-sm text-gray-400">📰 {hotspot.source}</div>}
              </a>
            ))}
          </div>
          <div ref={loadMoreRef} className="mt-8">
            ...
            {!hasMore && <div className="text-center py-8 text-gray-300 text-sm tracking-widest">已加载全部 {hotspots.length} 条热点</div>}
          </div>
        </>
      )}
    </div>
