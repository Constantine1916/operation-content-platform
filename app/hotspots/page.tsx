  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-lg text-gray-600 tracking-[0.2em] uppercase">Hotspot News</p>
      </div>
        ...
        <div className="flex gap-2 flex-wrap">
          <button
            ...
            className={`px-4 py-2 rounded-full text-lg tracking-widest uppercase transition-all ${
              selectedCategory === '' ? 'bg-gray-900 text-white' : 'bg-white text-gray-700 border border-gray-200 hover:border-gray-300'
            }`}
          >
            全部
          </button>
          {categories.slice(0, 12).map((cat) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-lg tracking-widest uppercase transition-all ${
                selectedCategory === cat ? 'bg-gray-900 text-white' : 'bg-white text-gray-700 border border-gray-200 hover:border-gray-300'
              }`}
            >
              {cat}
            </button>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-gray-600">暂无数据</div>
      ) : (
        <>
                ...
                <div className="flex items-start gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-3">
                      {hotspot.热度 && (
                        <span className="px-2 py-1 rounded-full text-[10px] tracking-widest uppercase bg-gray-50 text-gray-700 border border-gray-100">
                          🔥 {hotspot.热度}
                        </span>
                      )}
                      {hotspot.category && (
                        <span className="px-2 py-1 rounded-full text-[10px] tracking-widest uppercase bg-gray-50 text-gray-600 border border-gray-100">
                          {hotspot.category}
                        </span>
                      )}
                    ...
                    {hotspot.summary && (
                      <p className="text-lg text-gray-600 line-clamp-2 mb-3">{hotspot.summary}</p>
                    )}
                    {hotspot.source && (
          ...
          <div ref={loadMoreRef} className="mt-8">
            ...
            {!hasMore && hotspots.length > 0 && <div className="text-center py-8 text-gray-500 text-lg tracking-widest uppercase">已加载全部 {hotspots.length} 条热点</div>}
          </div>
        </>
      )}
    </div>
