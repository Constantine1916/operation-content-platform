...
export default function HotspotsPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-6 flex gap-2 flex-wrap">
        <button onClick={() => setSelectedCategory('')}
          className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all
            ${selectedCategory === '' ? 'bg-gray-900 text-white' : 'bg-white text-gray-700 border border-gray-300 hover:border-gray-600 hover:text-gray-900'}`}>
          全部
        </button>
        {categories.slice(0, 12).map(cat => (
          <button key={cat} onClick={() => setSelectedCategory(cat)}
            className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all
              ${selectedCategory === cat ? 'bg-gray-900 text-white' : 'bg-white text-gray-700 border border-gray-300 hover:border-gray-600 hover:text-gray-900'}`}>
            {cat}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        <div className="flex justify-center py-20"><div className="w-8 h-8 bg-gray-200 rounded-full animate-pulse"></div></div>
      ) : hotspots.length === 0 ? (
          ...
          <div className="space-y-3">
            {hotspots.map((hotspot) => (
              <a key={hotspot.id} href={hotspot.url} target="_blank" rel="noopener noreferrer"
                className="block bg-white border border-gray-200 rounded-2xl p-5 hover:border-gray-500 hover:shadow-md transition-all group">

                {/* 标签行 */}
                <div className="flex items-center gap-2 mb-2">
                  {hotspot.热度 && (
                    <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-orange-50 text-orange-600 border border-orange-200">
                      🔥 {hotspot.热度}
                    </span>
                  )}
                  {hotspot.category && (
                  ...
                  <span className="text-xs text-gray-500 ml-auto">{formatDate(hotspot.collected_date)}</span>
                </div>

                {/* 标题 - 最深 */}
                <h3 className="text-base font-semibold text-gray-900 group-hover:text-black line-clamp-2 mb-2">
                  {hotspot.title}
                </h3>

                {/* 摘要 - 次深 */}
                {hotspot.summary && (
                  <p className="text-sm text-gray-700 line-clamp-2 mb-2 leading-relaxed">{hotspot.summary}</p>
                )}

                {/* 来源 - 辅助 */}
                {hotspot.source && (
                  <div className="text-xs text-gray-500 mt-1">📰 {hotspot.source}</div>
                )}
              </a>
            ))}
          </div>
