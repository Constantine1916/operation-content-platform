...
export default function AiVideoPage() {
  ...
  const [stats, setStats] = useState({ total: 0, platforms: 0, models: 0 });
  ...
  const fetchData = () => {
    ...
    fetch(`/api/ai-video?${params.toString()}`)
      ...
      .then(data => {
        if (data.success) {
          setVideos(data.data || []);
          const platforms = new Set((data.data || []).map((v: VideoItem) => v.platform));
          const models = new Set((data.data || []).map((v: VideoItem) => v.model).filter(Boolean));
          setStats({ total: data.data?.length || 0, platforms: platforms.size, models: models.size });
        }
      })
      .catch(console.error)
  ...
  const copyPrompt = (id: string, prompt: string) => {
    navigator.clipboard.writeText(prompt).then(() => {
      ...
      setTimeout(() => {
        setCopied(prev => { const s = new Set(prev); s.delete(id); return s; });
      }, 2000);
    });
  };
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5">
          ...
          <p className="text-2xl font-normal text-gray-900">awesomevideoprompts</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-2xl p-5">
          <p className="text-[10px] text-gray-900 uppercase tracking-widest mb-2">获取时间</p>
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
              modelFilter === 'all' ? 'bg-gray-900 text-white' : 'bg-white border border-gray-200 text-gray-900 hover:bg-gray-50'
            }`}
          >
            全部模型
          </button>
          {allModels.map(m => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                modelFilter === m ? 'bg-gray-900 text-white' : 'bg-white border border-gray-200 text-gray-900 hover:bg-gray-50'
              }`}
            >
              {m}
            </button>
              ...
              return (
                <div key={video.id} className="px-6 py-5 hover:bg-gray-50 transition-colors">
                  ...
                  <div className="flex items-start gap-3 mb-2">
                    <span className="flex-shrink-0 w-6 h-6 rounded-full bg-gray-100 text-gray-900 flex items-center justify-center text-xs font-medium mt-0.5">
                      {i + 1}
                    </span>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          {video.model && (
                            <span className="text-[10px] bg-gray-100 text-gray-900 px-2 py-0.5 rounded-full font-medium">
                              {video.model}
                            </span>
                          )}
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        {video.author_url ? (
                          <a href={video.author_url} target="_blank" rel="noopener noreferrer" className="text-xs text-gray-900 hover:text-gray-700">
                        ...
                        {video.source_url && (
                          <a href={video.source_url} target="_blank" rel="noopener noreferrer" className="text-xs text-gray-900 hover:text-gray-700 ml-1">
                            ↗ source
                          </a>
                        )}
                      </div>
                      ...
                      {video.tags && video.tags.length > 0 && (
                        <div className="flex gap-1 mt-2 flex-wrap">
                          {video.tags.map(tag => (
                            <span key={tag} className="text-[10px] bg-gray-50 text-gray-900 px-2 py-0.5 rounded border border-gray-100">
                              #{tag}
                            </span>
                          ))}
                        </div>
                  ...
                  {!isExpanded ? (
                    <div
                      className="mt-2 ml-9 text-xs text-gray-900 bg-gray-50 rounded-xl p-3 cursor-pointer hover:bg-gray-100 transition-colors line-clamp-3"
                      onClick={() => toggleExpand(video.id)}
                    >
                      <pre className="whitespace-pre-wrap font-sans" style={{ fontFamily: 'inherit' }}>
                        {video.prompt.slice(0, 200)}{video.prompt.length > 200 ? '...' : ''}
                      </pre>
                      {video.prompt.length > 200 && (
                        <div className="text-center text-xs text-gray-900 mt-1 opacity-60">点击展开 ↓</div>
                      )}
                    </div>
                  ) : (
                    <div className="mt-2 ml-9">
                      <div className="text-xs text-gray-900 bg-gray-50 rounded-xl p-3">
                        <pre className="whitespace-pre-wrap font-sans text-xs leading-relaxed" style={{ fontFamily: 'inherit' }}>
                          {video.prompt}
                        </pre>
                      </div>
