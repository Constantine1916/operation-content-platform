...
async function fetchFavoriteItems(
  ...
  if (contentType === 'image') {
    const { data, error } = await db
      .from('ai_images')
      .select('id, task_id, prompt, url, width, height, "index", is_public, moderation_status, user_id, created_at, profiles(username, avatar_url)')
      .eq('is_public', true)
    ...
    return (data ?? []).map((img: any) => maskModeratedMedia({
      id: img.id,
      task_id: img.task_id,
      prompt: img.prompt,
      url: img.url,
  ...
  if (contentType === 'video') {
    const { data, error } = await db
      .from('ai_videos')
      .select('id, title, prompt, author, author_url, platform, model, video_url, source_url, is_public, moderation_status, created_at, user_id, profiles(username, avatar_url)')
      .eq('is_public', true)
      .in('moderation_status', ['active', 'nsfw'])
      .in('id', contentIds);
    ...
    return (data ?? []).map((video: any) => maskModeratedMedia({
      ...video,
