  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-lg text-gray-600 tracking-[0.2em] uppercase">Redbook Research</p>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden mb-6">
        <div className="px-6 py-4 border-b border-gray-50"><h2 className="text-lg tracking-[0.2em] text-gray-600 uppercase">搜索关键词</h2></div>
        <div className="divide-y divide-gray-50">
          {keywords.map((kw) => (
            <Link key={kw.keyword} href={`/xhs/search/${encodeURIComponent(kw.keyword)}`} className="flex items-center justify-between px-6 py-4 hover:bg-gray-50 transition-colors group">
              <div className="flex items-center gap-3">
                <span className="px-3 py-1.5 bg-gray-50 text-gray-800 text-lg rounded-full border border-gray-100 group-hover:border-gray-200 transition-colors">{kw.keyword}</span>
                <span className="text-lg text-gray-600">{kw.noteCount} 条笔记</span>
              </div>
          ...
          {keywords.length === 0 && <div className="px-6 py-12 text-center text-gray-600 text-xs">暂无搜索记录</div>}
        </div>
      </div>

      <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-50"><h2 className="text-lg tracking-[0.2em] text-gray-600 uppercase">TOP 笔记排行</h2></div>
        <div className="divide-y divide-gray-50">
          {topNotes.map((note, i) => (
            <div key={note.note_id} className="flex items-center gap-4 px-6 py-4">
              <span className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-lg ${i < 3 ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600'}`}>{i + 1}</span>
              <div className="flex-1 min-w-0">
                <div className="text-lg black truncate">{note.title || '无标题'}</div>
                <div className="text-[10px] text-gray-600 mt-0.5">@{note.nickname} · {note.keyword}</div>
              </div>
              <div className="flex items-center gap-3 text-[10px] text-gray-600 flex-shrink-0">
                ...
                {note.collect_like_ratio && <span className={`px-2 py-0.5 rounded-full ${note.collect_like_ratio > 50 ? 'bg-gray-100 text-gray-800' : 'bg-gray-50 text-gray-600'}`}>{note.collect_like_ratio}%</span>}
              </div>
            </div>
          ))}
        </div>
...
function StatCard({ label, value }: { label: string; value: number | string }) {
  return (
    <div className="bg-white border border-gray-100 rounded-2xl p-5">
      ...
      <p className="text-2xl font-normal text-gray-800">{value}</p>
    </div>
  );
}
