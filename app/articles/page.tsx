...
const platformColors: Record<string, string> = { xiaohongshu: 'bg-red-50 text-red-600 border-red-100', zhihu: 'bg-blue-50 text-blue-600 border-blue-100', wechat: 'bg-green-50 text-green-600 border-green-100', x: 'bg-gray-50 text-gray-600 border-gray-100', reddit: 'bg-orange-50 text-orange-600 border-orange-100' };
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-gray-400 text-sm">暂无文章</div>
      ) : (
        <>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {articles.map((article) => (
              <div
                ...
                className="bg-white border border-gray-100 rounded-2xl p-4 hover:border-gray-300 hover:shadow-md transition-all cursor-pointer group flex flex-col"
              >
                {/* Platform badge */}
                <div className="flex items-center justify-between mb-3">
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium border ${platformColors[article.platform] || 'bg-gray-50 text-gray-600 border-gray-100'}`}>
                    {platformNames[article.platform] || article.platform}
                  </span>
                  <span className="text-[10px] text-gray-300">{formatDate(article.created_at)}</span>
                </div>
                ...
                <div className="flex items-center justify-between mt-auto pt-2 border-t border-gray-50">
                  <span className="text-[10px] text-gray-400">{article.author ? `@${article.author}` : '—'}</span>
                  <span className="text-[10px] text-gray-400 group-hover:text-gray-600 transition-colors">阅读全文 →</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
            ...
            <div className="px-6 pt-6 pb-4 border-b border-gray-100 flex items-start justify-between gap-4 flex-shrink-0">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-2">
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium border ${platformColors[selectedArticle.platform] || 'bg-gray-50 text-gray-600 border-gray-100'}`}>
                    {platformNames[selectedArticle.platform] || selectedArticle.platform}
                  </span>
                  {selectedArticle.author && <span className="text-xs text-gray-400">@{selectedArticle.author}</span>}
                  <span className="text-xs text-gray-300">{formatDate(selectedArticle.created_at)}</span>
                </div>
                <h2 className="text-base font-semibold text-gray-900 leading-snug">{selectedArticle.title}</h2>
              </div>
              <button
                onClick={() => setSelectedArticle(null)}
                className="flex-shrink-0 w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors text-gray-500 hover:text-gray-800"
              >
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5">
              {selectedArticle.content ? (
                <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{selectedArticle.content}</p>
              ) : (
                <p className="text-sm text-gray-400 text-center py-8">暂无正文内容</p>
              )}
            </div>
          </div>
        </div>
